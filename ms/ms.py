...
def work_flow():
    ...
    if current_time.weekday() in range(0,5) and current_time.hour in range(7,17):
        return 20
    else:
        return 120
...
def main():
    ...
    username = Popen(["whoami"], stdout = PIPE).communicate()[0].strip()
    prefix ="/lscr2/andersenlab/" + username + "/mapping/" + slugify(args["<traits.tsv>"]) + "/"
    
    # Make Directories.
    try:
        os.mkdir(prefix)
        os.mkdir(prefix + "phenotypes")
        os.mkdir(prefix + "mappings_tsv")
        os.mkdir(prefix + "mappings_rdata")
        os.mkdir(prefix + "submission_scripts")
        os.mkdir(prefix + "logs")
    except:
        pass
